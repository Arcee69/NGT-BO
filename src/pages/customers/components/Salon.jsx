...
const Salon = () => {
  ...
  const allCustomersData = [
    ...
    const handlePageClick = (event) => {
        const newOffset = (event.selected * perPage) % allCustomersData.length;
        console.log(
      ...
      <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
        ...
        {allCustomersData?.length > 0 ? allCustomersData?.map((data, index) => (
            <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigate('/customers/details', { state: data }, window.scroll(0, 0))}>
                ...
                <td className='h-[70px] px-4'>
                    <p className='text-sm font-Mont text-dark-100 text-center'>{data?.businessName}</p>
                </td>
                <td className='h-[70px] px-4'>
                ...
                <td className='h-[70px] px-4'>
                  <div className='text-center'>
                      <p className='text-sm font-Mont text-dark-100'>{data?.phone}</p>
                      <p className='text-[11px] font-Mont font-bold text-[#8D8D8D]'>{data?.email}</p>
                  </div>
                </td>
                <td className='h-[70px] px-4'>
                ...
                <td className='h-[70px] px-4'>
                    <p className='text-sm font-Mont text-dark-100 text-center'>{data?.capacity}</p>
                </td>
