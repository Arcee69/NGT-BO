...
const Pending = ({ allPendingOrders, loading }) => {
  ...
  function timeAgo(dateString) {
    ...
    const handlePageClick = (event) => {
        const newOffset = (event.selected * perPage) % allPendingOrders?.length;
        console.log(
                    ...
                    <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigate('/orders/details', { state: data } )}>
                        ...
                        <td className='h-[70px] px-4'>
                            <div className={`rounded-lg h-8 flex justify-center items-center w-[78px]  bg-[#FFC60029]`}>
                                <p className={`text-sm font-Mont text-left font-semibold text-[#FFC600]`}>Pending</p>
                            </div>
                        </td>
                    </tr>
                )) : (
