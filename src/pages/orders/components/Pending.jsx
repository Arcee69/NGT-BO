...
const Pending = () => {
  ...
  const allPendingOrder = [
    ...
    const currentData = allPendingOrder.slice(itemOffset, endOffset);
    const pageCount = Math.ceil(allPendingOrder.length / perPage);


    //Change Page 
    const handlePageClick = (event) => {
        const newOffset = (event.selected * perPage) % allPendingOrder.length;
        console.log(
  ...
  return (
    <div className='mt-6'>
      ...
      <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
            ...
            {allPendingOrder?.length > 0 ? allPendingOrder?.map((data, index) => (
                <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100'>
                    ...
                    <td className='h-[70px] px-4'>
                        <div className={`rounded-lg h-8 flex justify-center items-center w-[78px]  bg-[#FFC60029]`}>
                            <p className={`text-sm font-Mont text-left font-semibold text-[#FFC600]`}>{data.status}</p>
                        </div>
                    </td>
                </tr>
            )) : (
