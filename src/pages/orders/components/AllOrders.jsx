...
const AllOrders = () => {
  ...
  const allOrderData = [
    ...
    const currentData = allOrderData.slice(itemOffset, endOffset);
    const pageCount = Math.ceil(allOrderData.length / perPage);


    //Change Page 
    const handlePageClick = (event) => {
        const newOffset = (event.selected * perPage) % allOrderData.length;
        console.log(
  ...
  return (
    <div className='mt-6'>
      ...
      <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
            ...
            {allOrderData?.length > 0 ? allOrderData?.map((data, index) => (
                <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' >
                    ...
                    <td className='h-[70px] px-4'>
                        <div className={`rounded-lg h-8 flex justify-center items-center ${data.status === 'Pending' && 'w-[78px]  bg-[#FFC60029]'} ${data.status === 'success' && ' w-[99px] bg-[#ECFDF5]'} ${data.status === 'Cancelled' && 'w-[92px] bg-[#FFF1F2]'} `}>
                            <p className={`text-sm font-Mont text-left font-semibold ${data.status === 'Pending' && 'text-[#FFC600]'} ${data.status === 'success' && 'text-[#10B981]'} ${data.status === 'Cancelled' && 'text-[#F43F5E]'} `}>{data.status}</p>
                        </div>
                    </td>
                </tr>
            )) : (
