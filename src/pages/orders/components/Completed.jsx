...
const Completed = () => {
  ...
  const allCompletedOrder = [
    ...
    const currentData = allCompletedOrder.slice(itemOffset, endOffset);
    const pageCount = Math.ceil(allCompletedOrder.length / perPage);


    //Change Page 
    const handlePageClick = (event) => {
        const newOffset = (event.selected * perPage) % allCompletedOrder.length;
        console.log(
  ...
  return (
    <div className='mt-6'>
      ...
      <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
            ...
            {allCompletedOrder?.length > 0 ? allCompletedOrder?.map((data, index) => (
                <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100'>
                    ...
                    <td className='h-[70px] px-4'>
                        <div className={`rounded-lg h-8 flex justify-center items-center w-[99px] bg-[#ECFDF5] `}>
                            <p className={`text-sm font-Mont text-left font-semibold text-[#10B981]`}>{data.status}</p>
                        </div>
                    </td>
                </tr>
            )) : (
